...
#     raw_rewards = torch.tensor(rewards, dtype=torch.float32).reshape(-1, group_size)
...
def train_grpo(
    ...
    for step, (problems, _, _, _, answers) in enumerate(islice(infinite_dataloader(train_dataloader), configs.n_grpo_steps), start=1):
        prompts = [prompt_template.format(question=problem) for problem in problems]
        outputs = model_inf.generate(prompts, sampling_params=sampling_params)
            ...
            for micro_step in range(configs.n_microbatches_per_rollout_batch + 1):
                ...
                if idx_start < configs.rollout_batch_size:
                    ...
                    if ((micro_step_count % configs.gradient_accumulation_steps == 0) or
                        ...
                        total_step_count += 1
                        logging.info(f"Step: {total_step_count}, step loss: {step_loss}, step reward:, etc.")
                        step_loss = 0

        if (step + 1) % configs.eval_every == 0:
            mode.eval()
